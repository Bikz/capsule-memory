...
const DEFAULT_ORG_ID = process.env.CAPSULE_DEFAULT_ORG_ID || 'demo-org';
const DEFAULT_PROJECT_ID = process.env.CAPSULE_DEFAULT_PROJECT_ID || 'demo-project';
const DEFAULT_SUBJECT_ID =
  process.env.CAPSULE_DEFAULT_SUBJECT_ID ||
  process.env.CAPSULE_MCP_SUBJECT_ID ||
  'local-mcp';
...
function withTenantDefaults(args = {}) {
  return {
    ...args,
    orgId: typeof args.orgId === 'string' && args.orgId ? args.orgId : DEFAULT_ORG_ID,
    projectId:
      typeof args.projectId === 'string' && args.projectId
        ? args.projectId
        : DEFAULT_PROJECT_ID,
    subjectId:
      typeof args.subjectId === 'string' && args.subjectId
        ? args.subjectId
        : DEFAULT_SUBJECT_ID
  };
}
...
async function callModelenceMethod(method, args = {}) {
  const payloadArgs = withTenantDefaults(args);
...
function formatMemorySummary(memory) {
  ...
  const createdAt = createdAtValue
    ...
    : 'unknown date';
  const subjectLine = memory.subjectId ? `\n  • subject: ${memory.subjectId}` : '';
  return `${pinnedBadge}${memory.content}\n  • id: ${memory.id}\n  • created: ${createdAt}${subjectLine}`;
}
...
function serializeMemory(memory) {
  ...
  return {
    ...
    ...(memory.orgId ? { orgId: memory.orgId } : {}),
  ...
  store: {
    input: {
      ...
      properties: {
        ...
        ttlSeconds: { type: 'integer', minimum: 1, maximum: 31536000 }
      }
    },
    output: {
      $schema: 'http://json-schema.org/draft-07/schema#',
...
const TOOL_DEFINITIONS = [
  {
    ...
    handler: async ({
      ...
      const structured = {
        ...
        subjectId: result.subjectId ?? DEFAULT_SUBJECT_ID,
      ...
      const textLines = [
        ...
        `Tenant: ${structured.orgId}/${structured.projectId} • subject ${structured.subjectId}`
      ];
      ...
      if (structured.expiresAt) {
        const expires = new Date(structured.expiresAt).toLocaleString();
        textLines.push(`Expires: ${expires}`);
      }
  ...
  },
  {
    ...
    handler: async ({ query, limit, subjectId, orgId, projectId }) => {
  ...
  },
  {
    ...
    handler: async ({ limit, subjectId, orgId, projectId, pinned, tag }) => {
      const data = await callModelenceMethod('memory.getMemories', {
        limit,
        subjectId,
        orgId,
        projectId,
  ...
  },
  {
    ...
    handler: async ({ id, pin, subjectId, orgId, projectId }) => {
      const result = await callModelenceMethod('memory.pinMemory', {
        id,
        pin,
        subjectId,
        orgId,
  ...
  },
  {
    ...
    handler: async ({ id, reason, subjectId, orgId, projectId }) => {
